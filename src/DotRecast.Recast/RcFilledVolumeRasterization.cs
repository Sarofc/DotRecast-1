...
namespace DotRecast.Recast
{
    ...
    public static class RcFilledVolumeRasterization
    {
        ...
        private static float[] IntersectBox(float[] rectangle, float[] vertices, float[][] planes)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    {
                        float dotNormalPoint = Vector3.Dot(RcVecUtils.Create(planes[j]), point);
        ...
        {
            y = 0.0f;
            float t = (planes[plane][3] - Vector3.Dot(RcVecUtils.Create(planes[plane]), point)) / planes[plane][1];
            Vector3 s = new Vector3(point.X, point.Y + t, point.Z);
            float u = Vector3.Dot(s, RcVecUtils.Create(planes[plane + 1])) - planes[plane + 1][3];
            if (u < 0.0f || u > 1.0f)
            {
                return false;
            }

            float v = Vector3.Dot(s, RcVecUtils.Create(planes[plane + 2])) - planes[plane + 2][3];
            if (v < 0.0f)
            {
                return false;
            }
