...
public class RecastDebugDraw : DebugDraw
{
    ...
    private unsafe void DrawMeshTile(DtNavMesh mesh, DtNavMeshQuery query, DtMeshTile tile, int flags)
    {
    ...
    void DrawPolyBoundaries(DtMeshTile tile, int col, float linew, bool inner)
    {
        ...
        Span<Vector3> tv = stackalloc Vector3[3];
        for (int i = 0; i < tile.data.header.polyCount; ++i)
        {
            DtPoly p = tile.data.polys[i];
    ...
    public unsafe void DebugDrawNavMeshPoly(DtNavMesh mesh, long refs, int col)
    {
        if (refs == 0)
        {
            return;
        ...
        {
            ref readonly DtOffMeshConnection con = ref tile.data.offMeshCons[ip - tile.data.header.offMeshBase];
